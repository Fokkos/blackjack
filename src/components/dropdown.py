...
class Dropdown:
    ...
    def draw(self, surface):
        """
        Draw the dropdown on the surface, including the options if the dropdown is open
        :param surface: The surface to draw the dropdown on
        """
        ...
        if self.is_open:
            ...
            for i, option in enumerate(self.options):
                option_rect = pygame.Rect(self.rect.x, self.rect.y + (i + 1) * MENU_INPUT_HEIGHT, self.rect.width,
                                          MENU_INPUT_HEIGHT)
    ...
    def select_option(self, mouse_pos):
        """
        Select an option from the dropdown based on the mouse position
        :param mouse_pos: The position of the mouse (done automatically by Pygame)
        After selecting an option, the dropdown will close and the selected option parameter will be updated
        """
